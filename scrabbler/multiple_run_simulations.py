...
def main():
    ...
    for i in range(n):
        ...
        player0 = Player(MCTSStrategy(num_rollouts=200), name="MCTS midgame")
        # player1 = Player(ShortMonteCarloSimStrategy(num_rollouts=500), name="Short Monte Midgame")
...
def main2():
    ...
    for i in range(n):
        # player0 = Player(midgame_strategy=BaselineStrategy(), endgame_strategy=ABStrategy(), name="AB Pruning Endgame")
        # player0 = Player(ShortMonteCarloSimStrategy(num_rollouts=10), name="Short Monte Midgame")
        player0 = Player(MCTSStrategy(num_rollouts=300), name="MCTS midgame")
        player1 = Player(ShortMonteCarloSimStrategy(num_rollouts=300), name="Short Monte Midgame")
        # player1 = Player(BaselineStrategy(), name="Baseline")
        # player1 = Player(BaselineStrategy(), name="Greedy Endgame")
        p0score, p1score = Simulation.simulate_game(player0, player1)
        p0scores.append(p0score)
        p1scores.append(p1score)
        # p0endgame_scores.append(p0score - player0.endgame_score)
        # p1endgame_scores.append(p1score - player1.endgame_score)
    print('time taken is.... ', time.time() - before)
...
if __name__ == "__main__":
    # firstGame0scores, firstGame1scores = main()
    secondGame0scores, secondGame1scores = main2()
    # print('FIRST GAME PLAYER 1: \n', firstGame0scores)
    # print('FIRST GAME PLAYER 2: \n', firstGame1scores)
    print('SECOND GAME PLAYER 1: \n', secondGame0scores)
    print('SECOND GAME PLAYER 2: \n', secondGame1scores)
