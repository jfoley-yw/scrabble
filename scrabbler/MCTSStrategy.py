...
class MCTSStrategy(Strategy):
    ...
    def select(self, game_state):
        ...
        while True:
            ...
            if unexplored:
                n = unexplored.pop()
                path.append(n)
                requisite_move = self.move_between_states[(game_state, n)]  # TODO - pinning down on why self.board isn't the same as game_state at points
    ...
    def find_next_states(self, game_state):  # move generator
        ...
        for move in moves:
            ...
            curr_bag = self.deduce_potential_bag(tupled_board)  # TODO - maybe have excluded tiles so next player cant get current players tiles
            next_rack = []
    ...
    #         next_rack = []
    #         for i in range(min(7, len(curr_bag))):
    #             new_letter = random.choice(curr_bag)
    #             next_rack.append(new_letter)
    #             curr_bag.remove(new_letter)
    ...
    #             next_game_state = MCTSgamestate(tupled_board, tupled_rack, not game_state.is_player1_turn,
